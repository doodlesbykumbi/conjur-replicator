...
const dbURL = "postgres://postgres@127.0.0.1:8432/postgres?replication=database"
const cacheURL = "localhost:6379"
...
func main() {
	...
	result = conn.Exec(context.Background(), "CREATE PUBLICATION pglogrepl_demo FOR TABLE secrets;")
	_, err = result.ReadAll()
	if err != nil {
	...
	for {
		...
		case pglogrepl.XLogDataByteID:
			...
			case *pglogrepl.InsertMessage:
				...
				err := rdb.Set(context.Background(), values["resource_id"].(string), base64.StdEncoding.EncodeToString(values["value"].([]byte)), 0).Err()
